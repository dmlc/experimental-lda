...
do
	...
	#run
	mpirun -v -cpus-per-proc 9 -np 8 --report-bindings -machinefile config/hosts dist/adoLDA --method "$METHOD" --testing-mode nt --num-sampling-threads $NST --num-topics $NUM_TOPICS --num-iterations $NUM_ITER --output-state-interval 1 --output-model $DIR_NAME --num-top-words 15 --dataset data/"$DATASET" | tee -a $DIR_NAME/log.txt
