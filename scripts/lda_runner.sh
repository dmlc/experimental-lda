...
do
	...
	dist/parallelLDA --method "$METHOD" --testing-mode net --alpha 50 --beta 0.1 --num-sampling-threads $NST --num-table-threads $NTT --num-topics $NUM_TOPICS --num-iterations $NUM_ITER --output-state-interval 1 --output-model $DIR_NAME --num-top-words 15 --dataset data/"$DATASET" | tee -a $DIR_NAME/log.txt
