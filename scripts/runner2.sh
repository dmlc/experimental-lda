...
do
	...
	dist/hdp --method "$METHOD" --testing-mode net --alpha 50 --beta 0.1 --gamma 10 --num-topics $NUM_TOPICS --num-iterations $NUM_ITER --output-state-interval 10 --output-model $DIR_NAME --num-top-words 15 --dataset data/"$DATASET" | tee -a $DIR_NAME/log.txt
