...
int model::save_model_others(std::string filename) const
{
    ...
    fout << "nst=" << nst << std::endl;
	fout << "ntt=" << ntt << std::endl;
	fout.close();
...
int model::init_est()
{
	...
	// locks
	mtx = new std::mutex[V];
	//ts = new workqueue::threadpool(nst);
	//tu = new workqueue::threadpool(ntt);
	cbuff = new circular_queue<delta>[nst*ntt];
	//for(int tn=0; tn<ntt; ++tn)
	//	cbuff[8*tn] = new circular_queue<delta>[nst];
	inf_stop = false;
	done = new bool[ntt];
	for(int tn=0; tn<ntt; ++tn)
		done[tn] = false;
	...
	// --- model variables ---
	nw = new int*[V];
	//nw = new std::atomic<int>*[V];
    for (int w = 0; w < V; w++)
	{
		nw[w] = new int[K];
        //nw[w] = new std::atomic<int>[K];
        for (int k = 0; k < K; k++)
		{
    	    nw[w][k] = 0;
        }
...
int model::estimate()
{
	...
	auto heldout_inf = std::async(std::launch::async, &model::inf_parallel, this);

	auto summer = 0; // sampling_threads[0].get();
	for (int tn = 0; tn < nst; ++tn)
		std::cout<< sampling_threads[tn].get();
...
int model::inf_parallel()
{
	std::mt19937 urng(nst);
	...
	do
	{
		for (int inf_liter = 0; inf_liter < inf_niters; ++inf_liter)
		{
			// for all newz_i
			for (int m = 0; m < newM; m++)
			{
				for (int n = 0; n < pnewdata->docs[m]->length; n++)
				{
					...
					for (int k = 0; k < K; k++)
					{
						psum += (newnd[m][k] + alpha) * (nw[w][k] + newnw[w][k] + beta) / (nwsum[k] + newnwsum[k] + Vbeta);
						p[k] = psum;
					}
		...
		likelihood.push_back(newllhw());
		std::cout << "Likelihood on held out documents: " << likelihood.back() <<" at time " << time_ellapsed.back() << std::endl;
	} while(!inf_stop);// (done.test_and_set());
...
double model::llhw() const
{
	...
	for (int m = 0; m < M; m++)
	{
		...
		sum += dsum / ptrndata->docs[m]->length;
		for (auto k = nds1[m].begin(); k != nds1[m].end(); ++k)
		{
			nd_m[k->first] = 0;
		}
