...
int utils::read_wordmap(std::string wordmapfile, std::map<std::string, unsigned> * pword2id)
{
	unsigned id;
	std::string temp;
	...
	if (!fin)
		throw std::runtime_error( "Error: Unable to write to file: " + wordmapfile );
	...
	{
		fin >> id;
		pword2id->insert(std::pair<std::string, unsigned>(temp, id));
	}
	fin.close();
...
int utils::write_wordmap(std::string wordmapfile, std::map<std::string, unsigned> * pword2id)
{
